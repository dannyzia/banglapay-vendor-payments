<?php
/**
 * Upay Payment Gateway for BanglaPay
 * Handles Upay mobile wallet payments with vendor-specific accounts
 *
 * @package BanglaPay
 * @since 1.0.0
 */
...
if (!class_exists('WC_Gateway_Vendor_Upay')) {
    class WC_Gateway_Vendor_Upay extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->method_title = __('Vendor Upay', 'banglapay-vendor-payments');
            $this->method_description = __('Accept Upay payments with vendor-specific accounts', 'banglapay-vendor-payments');
            $this->supports = array('products');
            
            $this->init_form_fields();
            $this->init_settings();
            
            $this->title = $this->get_option('title', __('Upay', 'banglapay-vendor-payments'));
            $this->description = $this->get_option('description', __('Pay with Upay', 'banglapay-vendor-payments'));
            ...
            // AJAX handlers
            add_action('wp_ajax_banglapay_submit_upay_payment', array($this, 'ajax_submit_payment'));
            add_action('wp_ajax_nopriv_banglapay_submit_upay_payment', array($this, 'ajax_submit_payment'));
        }
        ...
        public function init_form_fields() {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('Enable Upay Payment', 'banglapay-vendor-payments'),
                    'default' => 'yes'
                ),
                'title' => array(
                    ...
                    'default' => __('Upay', 'banglapay-vendor-payments'),
                    'desc_tip' => true,
                ),
                'description' => array(
                    ...
                    'default' => __('Pay securely using your Upay mobile wallet', 'banglapay-vendor-payments'),
                    'desc_tip' => true,
                ),
            );
        }
        ...
        public function process_payment($order_id) {
            ...
            if (empty($this->vendor_settings['upay_account']) || $this->vendor_settings['upay_enabled'] !== 'yes') {
                wc_add_notice(__('Upay payment is not available for this order.', 'banglapay-vendor-payments'), 'error');
                return array('result' => 'failure');
            }
            
            // Save payment method details to order
            $order->update_meta_data('_banglapay_vendor_id', $this->vendor_id);
            $order->update_meta_data('_banglapay_payment_method', 'upay');
            $order->update_meta_data('_banglapay_vendor_account', $this->vendor_settings['upay_account']);
            $order->save();
            
            // Update order status
            $order->update_status('pending', __('Awaiting Upay payment confirmation', 'banglapay-vendor-payments'));
        ...
        public function receipt_page($order_id) {
            ...
            $this->display_payment_page($order, array(
                ...
                'method_id' => 'upay',
                'color' => '#0078BE',
                'account_label' => __('Upay Account Number', 'banglapay-vendor-payments'),
                'instructions' => array(
                    __('Open your <strong>Upay app</strong> on your phone', 'banglapay-vendor-payments'),
                    __('Select <strong>"Send Money"</strong> or <strong>"Payment"</strong>', 'banglapay-vendor-payments'),
                    __('Enter the Upay number shown below', 'banglapay-vendor-payments'),
        ...
        public function ajax_submit_payment() {
            // Verify nonce
            if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'banglapay_upay_nonce')) {
            ...
            $order->update_meta_data('_upay_transaction_id', $transaction_id);
            $order->update_meta_data('_upay_sender_number', $sender_number);
            $order->update_meta_data('_upay_payment_notes', $notes);
            $order->update_meta_data('_upay_payment_date', current_time('mysql'));
            
            // Update order status
            $order->update_status('on-hold', sprintf(
                /* translators: %s: transaction ID */
                __('Upay payment submitted. Transaction ID: %s', 'banglapay-vendor-payments'),
                $transaction_id
            ));
        ...
        public function thankyou_page($order_id) {
            ...
            $transaction_id = $order->get_meta('_upay_transaction_id');
            ...
            if ($transaction_id) {
                ...
                echo '<p>' . esc_html__('Thank you! We have received your Upay payment information.', 'banglapay-vendor-payments') . '</p>';
