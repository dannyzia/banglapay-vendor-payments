<?php
/**
 * bKash Payment Gateway for BanglaPay
 * Handles bKash mobile wallet payments with vendor-specific accounts
 *
 * @package BanglaPay
 * @since 1.0.0
 */
...
if (!class_exists('WC_Gateway_Vendor_Bkash')) {
    class WC_Gateway_Vendor_Bkash extends WC_Payment_Gateway {
        ...
        public function __construct() {
            $this->id = 'vendor_bkash';
            $this->icon = '';
            $this->has_fields = true;
            $this->method_title = __('Vendor bKash', 'banglapay-vendor-payments');
            $this->method_description = __('Accept bKash payments with vendor-specific accounts', 'banglapay-vendor-payments');
            $this->supports = array('products');
            
            $this->init_form_fields();
            $this->init_settings();
            
            $this->title = $this->get_option('title', __('bKash', 'banglapay-vendor-payments'));
            $this->description = $this->get_option('description', __('Pay with bKash', 'banglapay-vendor-payments'));
            ...
            // AJAX handlers
            add_action('wp_ajax_banglapay_submit_bkash_payment', array($this, 'ajax_submit_payment'));
            add_action('wp_ajax_nopriv_banglapay_submit_bkash_payment', array($this, 'ajax_submit_payment'));
        }
        ...
        public function init_form_fields() {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('Enable bKash Payment', 'banglapay-vendor-payments'),
                    'default' => 'yes'
                ),
                'title' => array(
                    ...
                    'default' => __('bKash', 'banglapay-vendor-payments'),
                    'desc_tip' => true,
                ),
                'description' => array(
                    ...
                    'default' => __('Pay securely using your bKash mobile wallet', 'banglapay-vendor-payments'),
                    'desc_tip' => true,
                ),
            );
        }
        ...
        public function process_payment($order_id) {
            ...
            if (empty($this->vendor_settings['bkash_account']) || $this->vendor_settings['bkash_enabled'] !== 'yes') {
                wc_add_notice(__('bKash payment is not available for this order.', 'banglapay-vendor-payments'), 'error');
                return array('result' => 'failure');
            }
            
            // Save payment method details to order
            $order->update_meta_data('_banglapay_vendor_id', $this->vendor_id);
            $order->update_meta_data('_banglapay_payment_method', 'bkash');
            $order->update_meta_data('_banglapay_vendor_account', $this->vendor_settings['bkash_account']);
            $order->save();
            
            // Update order status
            $order->update_status('pending', __('Awaiting bKash payment confirmation', 'banglapay-vendor-payments'));
        ...
        public function receipt_page($order_id) {
            ...
            $this->display_payment_page($order, array(
                ...
                'method_id' => 'bkash',
                'color' => '#E2136E',
                'account_label' => __('bKash Account Number', 'banglapay-vendor-payments'),
                'instructions' => array(
                    __('Open your <strong>bKash app</strong> on your phone', 'banglapay-vendor-payments'),
                    __('Select <strong>"Send Money"</strong> or <strong>"Payment"</strong>', 'banglapay-vendor-payments'),
                    __('Enter the bKash number shown below', 'banglapay-vendor-payments'),
        ...
        public function ajax_submit_payment() {
            // Verify nonce
            if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'banglapay_bkash_nonce')) {
            ...
            $order->update_meta_data('_bkash_transaction_id', $transaction_id);
            $order->update_meta_data('_bkash_sender_number', $sender_number);
            $order->update_meta_data('_bkash_payment_notes', $notes);
            $order->update_meta_data('_bkash_payment_date', current_time('mysql'));
            
            // Update order status
            $order->update_status('on-hold', sprintf(
                /* translators: %s: transaction ID */
                __('bKash payment submitted. Transaction ID: %s', 'banglapay-vendor-payments'),
                $transaction_id
            ));
        ...
        public function thankyou_page($order_id) {
            ...
            $transaction_id = $order->get_meta('_bkash_transaction_id');
            ...
            if ($transaction_id) {
                ...
                echo '<p>' . esc_html__('Thank you! We have received your bKash payment information.', 'banglapay-vendor-payments') . '</p>';
